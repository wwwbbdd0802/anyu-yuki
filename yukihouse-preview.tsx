    ...
    <div style={{backgroundColor:BG,color:"#fff",minHeight:"100vh",fontFamily:"'Noto Sans TC','PingFang TC',sans-serif"}}>
      ...
      <section style={{borderTop:`1px solid ${BD}`,padding:"32px 20px 20px",textAlign:"center"}}>
        ...
        <div style={{display:"flex",gap:10,justifyContent:"center"}}>
          {[{lb:"LINE 分享",url:`https://social-plugins.line.me/lineit/share?url=${encodeURIComponent(window.location.href)}`},{lb:"FB 分享",url:`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(window.location.href)}`}].map(({lb,url})=>(
...
function Sale({setPage,prop,setProp}){
  ...
  // 上傳謄本 → 由 AI 自動帶入欄位（需 API；於 claude.ai 預覽可直接運作，
  // 部署到 Vercel 時需搭配後端 API route，詳見部署說明）
  async function onUpload(e){
    ...
    try{
      ...
      const content=[
        ...
        {type:"text",text:"這是台灣不動產謄本（土地/建物登記第二類謄本）。請擷取以下欄位，只輸出 JSON，不要任何說明或 markdown：{\"city\":\"縣市如台北市\",\"district\":\"行政區如大安區\",\"ping\":建物總面積換算坪數的數字,\"acqYear\":取得/登記年份西元數字,\"landCurrentValue\":公告土地現值總額數字或null}。找不到的欄位填 null。"}
      ];
      ...
      setUp({loading:false,msg:"⚠ 自動辨識失敗，請手動填寫（部署後需設定 API 才能使用此功能）"});
    }
  }
...
function Lead({setPage}){
  ...
  function submit(){
    ...
    fetch(URL,{method:"POST",mode:"no-cors",headers:{"Content-Type":"application/json"},body:JSON.stringify({...f,timestamp:new Date().toISOString()})});
    */
    setSent(true);
  }
  if(sent) return(
